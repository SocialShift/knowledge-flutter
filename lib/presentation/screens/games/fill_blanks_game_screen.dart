...
class FillBlanksGameScreen extends ConsumerStatefulWidget {
  const FillBlanksGameScreen({Key? key}) : super(key: key);

  @override
  ConsumerState<FillBlanksGameScreen> createState() =>
      _FillBlanksGameScreenState();
}

class _FillBlanksGameScreenState extends ConsumerState<FillBlanksGameScreen> {
  ...
  void _loadQuestions() async {
    ...
    try {
      final questions =
          await ref.read(gameQuestionsProvider(kFillBlanksGameType).future);
  ...
  // Helper to format question with blank space
  String _formatQuestionWithBlank(String question) {
    if (!question.contains('_____')) {
      // If question doesn't already have blanks, add them
      return question.replaceAll('[BLANK]', '_____');
    }
    return question;
  }
}
